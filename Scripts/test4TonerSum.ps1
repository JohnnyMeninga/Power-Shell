...
d
            ...
            if ($blackTonerLevel -eq 0) {
                $message += "Black toner is completely finished and needs replacing.`n"
            }
            if ($cyanTonerLevel -eq 0) {
                $message += "Cyan toner is completely finished and needs replacing.`n"
            }
            if ($magentaTonerLevel -eq 0) {
                $message += "Magenta toner is completely finished and needs replacing.`n"
            }
            if ($yellowTonerLevel -eq 0) {
                $message += "Yellow toner is completely finished and needs replacing.`n"
            }
            ...
            if ($message) {
                ...
                $messageSubject = "Toner Replacement Notification for $name"
                $messageBody = "Printer: $name ($ip)`n$message"
